...
?>
	...
	<div class="filter-field-category-multi multiple-select">
		<h3 class="filter-title__hover">
			<?php echo JText::_('MOD_JSHOP_EFILTER_FIELDS_PRODUCT_CATEGORY_TITLE'); ?>
		</h3>
		
		<select class="category-ul__check" name="category[]" multiple="multiple">		
		<?php
			if($categories) {
				foreach($categories as $category) {
					...
					echo "<option value='".$category->category_id."'".$filter->title.">".$category->name."</option>";
				}
			}
		?>		
		</select>		
